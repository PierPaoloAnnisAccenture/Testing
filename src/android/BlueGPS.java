...
public class BlueGPS extends CordovaPlugin {
    ...
    private void setupConfigurationMap(JSONObject configurationJSON) {
        ...
        try {
            ...
            if (configurationJSON.has("show")) {
                JSONObject showmapNav = configurationJSON.getJSONObject("show");
            ...
            if (configurationJSON.has("style")) {
                JSONObject styleNav = configurationJSON.getJSONObject("style");
                if (styleNav.has("icons")) {
                    IconStyle iconsStyleNav = new IconStyle();

                    JSONObject iconsNav = styleNav.getJSONObject("icons");

                    if (iconsNav.has("opacity"))
                        iconsStyleNav.setOpacity(iconsNav.getDouble("opacity"));
                    ...
                    if (iconsNav.has("vAlign"))
                        iconsStyleNav.setVAlign(iconsNav.getString("vAlign"));
                    if (iconsNav.has("followZoom"))
                        iconsStyleNav.setFollowZoom(iconsNav.getBoolean("followZoom"));

                    mapStyleNav.setIcons(iconsStyleNav);

                }

                if (styleNav.has("indication")) {
                ...
                if (styleNav.has("navigation")) {
                    ...
                    if (navigationNav.has("velocityOptions")) {
                        ...
                        JSONObject velocityOptions = navigationNav.getJSONObject("velocityOptions");
                        if (velocityOptions.has("foot")) {
                            velocityOptionsNav.put("foot", velocityOptions.getDouble("foot"));
                        }
                        if (velocityOptions.has("bike")) {
                            velocityOptionsNav.put("bike", velocityOptions.getDouble("bike"));
                        }
                ...
                configurationMap.setStyle(mapStyleNav);
            }
            ...
            if (configurationJSON.has("visualization")) {
                //TODO
                //configurationMap.setVisualization();
            }
    ...
    private void navigationOld(JSONArray args) throws JSONException {
        configurationMap = new ConfigurationMap();
...
//                mapStyleNav.setIndication(indicationStyleNav);
    ...
    private ConfigurationMap setupConfigurationMap(boolean isNavigation) {
        ...
        navigationStyle.setStrokeWidthMeter(10.0);
        navigationStyle.setStroke("#dc8731");
        navigationStyle.setNavigationStep(1.5);
        navigationStyle.setShowVoronoy(false);


        navigationStyle.setAutoZoom(true);

        Map<String, Double> velocityOptions = new HashMap<>();
        velocityOptions.put("foot", 4.0);
        velocityOptions.put("bike", 10.0);
...
*/
    ...
    private PluginResult getPoiTouchResult(PluginResult.Status status, String payload) {
        ...
        Type cType = new TypeToken<Position>() {
        }.getType();
        ...
        try {
            ...
            Type poiType = new TypeToken<PoiField>() {
            }.getType();
